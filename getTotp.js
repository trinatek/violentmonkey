(function() {
  ...
  async function generateTotp(secretKey, hashAlgo) {
    try {
      ...
      return formatOtp(await getHmac(cryptoKey, timeBuffer));
    } catch (error) {
  ...
  async function getHmac(cryptoKey, timeBuffer) {
    return new Uint8Array(await crypto.subtle.sign("HMAC", cryptoKey, timeBuffer));
  }
  ...
  function formatOtp(hmac) {
    return generateOtp(hmac).toString().padStart(6, "0");
  }
  ...
  // Expose getTotp to the global scope
